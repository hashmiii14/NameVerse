...
export const metadata: Metadata = {
  title: 'Arabic Girl Names — Meanings, Etymology & Roots | NameVerse',
  description: 'Explore thousands of authentic Arabic girl names with classical etymology, meanings, and linguistic roots.',
  alternates: {
    canonical: 'https://name-verse.vercel.app/arabic-girl-names',
  },
};

export default async function ArabicGirlNamesPage({ searchParams }: PageProps) {
  ...
  const { results, total } = queryNamesServer(
    { gender: 'Female', origin: 'Arabic', letter },
    pageSize,
    offset
  );
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      <div className="max-w-2xl mx-auto">
        <SearchBar placeholder="Search Arabic girl names..." />
      </div>
      ...
      <nav className="text-xs text-slate-500 flex items-center gap-1.5 font-medium">
        ...
        <span>/</span>
        <span className="font-bold text-rose-800">Arabic Girl Names</span>
      </nav>
      ...
      <div className="bg-white border border-slate-200 rounded-3xl p-6 sm:p-10 space-y-3 shadow-xs">
        ...
        <p className="text-xs sm:text-sm text-slate-600 max-w-2xl leading-relaxed">
          Discover authentic Arabic female personal names. Showing <span className="font-bold text-slate-900">{total.toLocaleString()}</span> documented names.
        </p>
      </div>
      ...
      <div className="flex flex-wrap items-center justify-center gap-1.5 p-3 rounded-2xl bg-white border border-slate-200 shadow-2xs">
        ...
        {ALPHABET.map(char => (
          <Link
            key={char}
            href={`/arabic-girl-names?letter=${char}`}
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-2 pt-4">
          {currentPage > 1 && (
            <Link
              href={`/arabic-girl-names?page=${currentPage - 1}${letter !== 'All' ? `&letter=${letter}` : ''}`}
